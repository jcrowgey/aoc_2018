 ...
 *
 * https://github.com/rust-unofficial/too-many-lists/blob/master/lists/src/silly1.rs
 *
 * Thank you for the wonder article on linked lists in Rust and code snippets!  */
use std::fmt;
    ...
    std::fmt::Debug + Copy
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let mut left_fmt = Vec::new();
        let mut right_fmt = Vec::new();
        let mut left_node = self.left.head.as_ref();
        let mut right_node = self.right.head.as_ref();

        loop {
            match left_node {
                None => break,
                Some(n) => {
                    left_fmt.push(n.elem.clone());
                    left_node = n.next.as_ref();
                },
            };
        }
        left_fmt.reverse();
        ...
        loop {
            match right_node {
                None => break,
                Some(n) => {
                    right_fmt.push(n.elem.clone());
                    right_node = n.next.as_ref();
                },
            };
        }

        write!(f, "{:?} _ {:?}", left_fmt, right_fmt)
    }
}
